module HitProductsHelper
    def product_json(date)
        date = date.to_json(:methods => [:productId, :order, :dateAgo, :imageUrl, :isSoldOut, :isDeleted, :isTitleChanged, :shortUrl], :except => [:id, :created_at, :updated_at, :website, :is_sold_out, :image_url, :redirect_url, :dead_check, :is_title_changed, :shortDate, :date])
    ...
    def attr_refactory(data)
        ...
        data.each do |currentData|
            currentData.productId = currentData.product_id
